...
export const useMoodBoard = (guideImages: MoodBoardImage[]) => {
  ...
  useEffect(() => {
    if (guideImages && guideImages.length > 0) {
      const serverImages: MoodBoardImage[] = guideImages.map((image: any) => ({
        id: image.id,
        preview: image.url,
      ...
      if (currentImages.length === 0) {
        ...
        serverImages.forEach((serverImage) => {
          const clientIndex = mergedImages.findIndex(
            (clientImg) => clientImg.storageId === serverImage.storageId,
          );
  ...
  const removeImage = async (imageId: string) => {
    ...
    setValue("images", updatedImages);
    toast.success("Image removed successfully from mood board");
  };
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (imageFiles.length === 0) {
      toast.error("Please drop images only");
      return;
    }
  ...
  // }, [images]);
  useEffect(() => {
    const updatedPendingImages = async () => {
      const currentImages = getValues("images");
